...
### 数据库自动迁移规则
- 任何数据库表结构、字段长度、默认值、可空性、索引、约束、枚举含义或持久化数据格式变更，都必须随代码一起提交 Alembic migration。
- 禁止只修改 ORM/model 或把“让用户手动执行 SQL”作为正常交付方案；部署必须能通过 `python -m alembic upgrade head` 自动完成结构修复。
- 迁移应包含必要的数据回填、兼容旧数据的处理和可审阅的 downgrade 风险说明，避免线上部署卡在已有脏数据或旧 schema 上。
