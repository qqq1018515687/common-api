...
def main():
    ...
    logger.info(f"临时文件保留时间: {args.temp_hours} 小时")
    logger.info(f"任务文件保留时间: {args.task_days} 天")
    logger.info("=" * 60)
    ...
    try:
        ...
        try:
            ...
            logger.info("清理结果汇总")
            logger.info("=" * 60)
            logger.info(f"临时文件清理: {result['temp_cleanup']}")
            logger.info(f"孤立头像清理: {result['avatar_cleanup']}")
            logger.info(f"任务文件清理: {result['task_cleanup']}")
            logger.info(f"存储统计: {result['storage_stats']}")
