...
class InputData(BaseModel):
    ...
    file_list: Optional[List[File]] = Field(
        default=None, description="文件列表（提示词增强使用）"
    )
    file_key: Optional[str] = Field(
        default=None,
        description="对象存储文件键（delete_upload/storage_management 使用）",
    )
    category: Optional[str] = Field(
        default=None, description="文件分类：avatar/upload/temp 或 storage 分类"
    )
    prefix: Optional[str] = Field(
        default=None, description="对象存储前缀筛选（storage_management 使用）"
    )
    continuation_token: Optional[str] = Field(
        default=None, description="对象存储分页 token（storage_management 使用）"
    )
    dry_run: Optional[bool] = Field(
        default=None, description="对象储存清理试运行（storage_management 使用）"
    )
    include_avatars: Optional[bool] = Field(
        default=None, description="对象储存清理是否包含头像（storage_management 使用）"
    )
    ...
    content_type: Optional[str] = Field(
        default=None, description="对象存储上传文件 MIME 类型"
    )
    size: Optional[int] = Field(default=None, description="对象存储上传文件大小")
    file_content_base64: Optional[str] = Field(
        default=None, description="对象存储上传文件 base64 内容"
    )
    convert_to_pdf: bool = Field(
        default=False, description="对象存储上传时是否将 Office 文档转换为 PDF"
    )
    asset_mode: bool = Field(
        default=False, description="对象存储是否按站点长期资源处理"
    )
    expires_in: Optional[int] = Field(
        default=None, description="对象存储签名 URL 过期秒数"
    )
    ...
    user_id: Optional[str] = Field(
        default=None, description="用户 ID（save/history/update_user/delete_user 使用）"
    )
    runninghub_link: Optional[str] = Field(
        default=None, description="RunningHub 链接（save 使用）"
    )
    tool_type: Optional[str] = Field(
        default=None,
        description="工具类型：reverse_image/translate_doubao/translate_flash/prompt_enhance",
    )
    prompt: Optional[str] = Field(
        default=None, description="提示词/待翻译文本（tool 使用）"
    )
    ...
    current_target: Optional[dict] = Field(
        default=None, description="Agent 意图判断当前已选目标"
    )
    agent_preferences: Optional[dict] = Field(
        default=None, description="Agent 生成偏好与模型偏好"
    )
    capability_hash: Optional[str] = Field(default=None, description="Agent 能力表哈希")
    capability_manifest_url: Optional[str] = Field(
        default=None, description="Agent 能力表获取地址"
    )
    ...
    agent_steps: Optional[List[dict]] = Field(
        default=None, description="Agent 执行步骤"
    )
    agent_run_updates: Optional[dict] = Field(
        default=None, description="Agent Run 更新字段"
    )
    ...
    end_time: Optional[int] = Field(
        default=None, description="查询结束时间戳（毫秒，13位整数）"
    )
    ...
    task_data: Optional[dict] = Field(
        default=None, description="任务数据（create_task 使用）"
    )
    task_updates: Optional[dict] = Field(
        default=None, description="任务更新数据（update_task 使用）"
    )

    # 系统通知相关字段
    notification_id: Optional[str] = Field(
        default=None,
        description="通知ID（update_notification/delete_notification 使用）",
    )
    notification_data: Optional[dict] = Field(
        default=None,
        description="通知数据（create_notification/update_notification 使用）",
    )
    ...
    announcement_id: Optional[str] = Field(
        default=None, description="公告ID（update/disable 使用）"
    )
    announcement_data: Optional[dict] = Field(
        default=None, description="公告数据（create/update 使用）"
    )
    target_audience: Optional[str] = Field(
        default=None, description="目标用户：all/logged_in/guest/admin"
    )

    # 团队余额相关字段
    amount: Optional[float] = Field(
        default=None, description="金额（充值/扣费/退款使用）"
    )
    ...
    original_record_id: Optional[str] = Field(
        default=None, description="原消费记录ID（退款用）"
    )
    ...
    error_response: Optional[dict] = Field(
        default=None,
        description="RunningHub 错误响应数据（runninghub_error_analysis 使用）",
    )

    # Billing 资金扣费相关字段
    credit_type: Optional[str] = Field(
        default=None, description="资金类型：personal_gold/personal_silver/team_gold"
    )
    idempotency_key: Optional[str] = Field(
        default=None, description="幂等键（deduct/refund/settle 必传）"
    )
    service_secret: Optional[str] = Field(
        default=None, description="服务密钥（billing 操作必传）"
    )
    final_amount: Optional[float] = Field(
        default=None, description="结算金额（settle 使用）"
    )
    billing_metadata: Optional[dict] = Field(
        default=None,
        description="billing 元数据（main 透传，含 title/workflow/model 等信息）",
    )
...
class GlobalState(BaseModel):
    ...
    user_id: Optional[str] = Field(
        default=None,
        description="用户 ID（save/update_user/delete_user/create_task/list_tasks 使用）",
    )
    runninghub_link: Optional[str] = Field(
        default=None, description="RunningHub 链接（save 使用）"
    )
    ...
    # RunningHub 错误分析相关字段
    error_response: Optional[dict] = Field(
        default=None, description="RunningHub 错误响应数据"
    )
...
class CheckNeedTagsOutput(BaseModel):
    ...
    image_url: Optional[str] = Field(
        default=None, description="图像URL（需要生成标签时）"
    )
...
class UploadInput(BaseModel):
    ...
    category: Optional[str] = Field(
        default=None, description="文件分类：avatar/upload/temp"
    )
...
class GetTaskInput(BaseModel):
    ...
    user_id: Optional[str] = Field(
        default=None, description="用户ID（必须是已注册的活跃用户）"
    )
...
class ListTasksInput(BaseModel):
    ...
    user_id: Optional[str] = Field(
        default=None,
        description="用户ID（可选，至少提供 user_id 或 team_id 之一，admin 可不传）",
    )
    team_id: Optional[str] = Field(
        default=None,
        description="团队ID筛选（可选，至少提供 user_id 或 team_id 之一，admin 可不传）",
    )
...
class CountTasksStatsInput(BaseModel):
    """统计任务数量节点的输入"""
...
class CountTasksStatsOutput(BaseModel):
    """统计任务数量节点的输出"""
...
class TaskRouteInput(BaseModel):
    ...
    # 时间范围查询字段
    start_time: Optional[int] = Field(
        default=None, description="查询开始时间戳（毫秒）"
    )
...
class FormatResponseInput(BaseModel):
    ...
    response_data: Optional[dict] = Field(
        default=None, description="直接返回的响应数据（团队余额等节点使用）"
    )
...
class UnpackInputDataOutput(BaseModel):
    ...
    continuation_token: Optional[str] = Field(
        default=None, description="对象存储分页 token"
    )
    dry_run: Optional[bool] = Field(default=None, description="对象储存清理试运行")
    include_avatars: Optional[bool] = Field(
        default=None, description="对象储存清理是否包含头像"
    )
    folder_name: Optional[str] = Field(default=None, description="对象存储文件夹名称")
    file_name: Optional[str] = Field(default=None, description="对象存储上传文件名")
    content_type: Optional[str] = Field(
    ...
    billing_metadata: Optional[dict] = Field(
        default=None, description="billing 元数据（main 透传）"
    )
    metadata: Optional[dict] = Field(
        default=None,
        description="通用元数据（含 billing_metadata 嵌套结构，main 透传）",
...
class ToolRouteInput(BaseModel):
    ...
    tool_type: str = Field(
        ..., description="工具类型：reverse_image/translate_doubao/translate_flash"
    )
...
class SystemNotificationInput(BaseModel):
    ...
    notification_data: Optional[dict] = Field(
        default=None, description="通知数据（create/update 使用）"
    )
    current_time: Optional[int] = Field(
        default=None, description="当前时间戳（用于筛选有效通知）"
    )
...
class AnnouncementInput(BaseModel):
    ...
    operation_type: str = Field(
        ..., description="操作类型：get_active_popup/get_all/create/update/disable"
    )
    current_time: Optional[int] = Field(
        default=None, description="当前时间戳（用于筛选有效公告）"
    )
    target_audience: Optional[str] = Field(
        default="all", description="目标用户：all/logged_in/guest/admin"
    )
    announcement_id: Optional[str] = Field(
        default=None, description="公告ID（update/disable 使用）"
    )
...
class BillingRouteInput(BaseModel):
    ...
    operation_type: Optional[str] = Field(
        default=None,
        description="操作类型：get_balance/deduct/refund/settle/list_records/get_records",
    )
    user_id: Optional[str] = Field(default=None, description="用户ID")
    ...
    task_id: Optional[str] = Field(
        default=None, description="关联任务ID（deduct 可选）"
    )
    description: Optional[str] = Field(default=None, description="操作描述")
    original_record_id: Optional[str] = Field(
        default=None, description="原扣费记录ID（refund/settle 使用）"
    )
    final_amount: Optional[float] = Field(
        default=None, description="结算金额（settle 使用）"
    )
    ...
    operator_user_id: Optional[str] = Field(default=None, description="操作者用户ID")
    billing_metadata: Optional[dict] = Field(
        default=None, description="billing 元数据（含 title/workflow/model 等）"
    )
    metadata: Optional[dict] = Field(
        default=None,
        description="通用元数据（含 billing_metadata 嵌套结构，main 透传）",
...
class BillingRefundInput(BaseModel):
    ...
    service_secret: Optional[str] = Field(default=None, description="服务密钥")
    amount: Optional[float] = Field(
        default=None, description="退款金额（不传则全额退款）"
    )
